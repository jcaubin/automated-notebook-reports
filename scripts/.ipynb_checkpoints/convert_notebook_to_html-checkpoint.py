import subprocess
import os

def run_and_convert_to_html(notebook_folder, notebook_name):
    """
    Executes a Jupyter Notebook, saves it as an HTML file without input cells,
    and stores it in the 'reports' folder, which is at the same level as the notebook folder.

    Parameters:
        notebook_folder (str): Path to the folder containing the notebook.
        notebook_name (str): Name of the notebook file to execute (e.g., 'example_notebook.ipynb').
    """
    try:
        # Construct the full path to the notebook
        notebook_path = os.path.join(notebook_folder, notebook_name)

        # Define the output folder ('reports' is at the same level as 'notebooks')
        reports_folder = os.path.join(os.path.dirname(notebook_folder), 'reports')

        # Ensure the 'reports' folder exists
        os.makedirs(reports_folder, exist_ok=True)

        # Construct the output HTML path
        output_path = os.path.join(reports_folder, f"{os.path.splitext(notebook_name)[0]}.html")

        # Run the nbconvert command
        subprocess.run([
            'jupyter', 'nbconvert', '--to', 'html', '--no-input', 
            '--output', output_path, notebook_path
        ], check=True)

        print(f"Notebook executed and saved as HTML: {output_path}")

    except subprocess.CalledProcessError as e:
        print(f"An error occurred during nbconvert: {e}")
    except Exception as e:
        print(f"An error occurred: {e}")

# Example usage
notebook_folder = '../notebooks'  # Relative path to the Notebook folder
notebook_name = 'analysis_purchases.ipynb'

run_and_convert_to_html(notebook_folder, notebook_name)